...
6. **Optimize column ordering** — per-entity greedy search: for each entity with FK columns scattered between non-FK columns, run the full pipeline twice (declared order vs PK→FK→other) and keep whichever yields fewer crossings (tie-break: shorter `totalVLength`; fallback: declared). Decisions accumulate, so later entities see earlier winners and catch cascade effects through the sibling tiebreak. Opt-out: `@layout { preserve_order }` global or `preserve_order entity1, entity2` per-entity.
...
- **Column ordering is a per-entity search, not a fixed rule.** An earlier always-reorder rule (PK → FK → other) regressed star-default by 3 cells of V length while improving snowflake by 2 — a wash across shapes. The current scheme tries both orderings per "interesting" entity, runs the full pipeline for each, and keeps whichever yields fewer crossings (tie-break: shorter `totalVLength`; fallback: declared). Determinism preserved since the metric is deterministic. Cost: 2× layout time per candidate entity, still sub-millisecond at typical scale. Opt-out via `@layout { preserve_order [entities] }`.
- **`crossings` is the load-bearing optimization metric, `totalVLength` is the tie-breaker.** Earlier visual-clustering predicates (`bendAdjacent`, `portAdjacent`) measured glyph fusion that didn't correlate with structural cost; they stayed stable when the layout actually improved, and an optimization that drove them down (channel padding) added dead space without reducing crossings. Reachable via `routeStats(layout)`.
- **Barycenter Y-offset pass in `assignColumnYs`.** After the greedy + col-centered initial pack, each entity is pulled toward port-row alignment with its neighbors: `idealY = avg(neighbor.Y + neighbor.portRow − this.portRow)` over its refs, then each col is re-packed under order + min-gap-1 constraints via PAVA (pool adjacent violators) — the L2-optimal monotone solution. Iterates 4 passes alternating direction; iterative averaging drifts downward (round-half-up bias), so the last step normalizes min Y to 0 to keep the canvas anchored. Dissolves short V trunks that would otherwise produce confusing glyph fusion at channel boundaries (the original hero's `├┬┴┤` `provider_id` ↔ `payer_id` artifact). Knock-on: the column-reorder optimizer keeps declared order more often because barycenter handles alignment, and total V length drops across every snapshot fixture (questionnaire went from 4 crossings + 95 V-length to 0 + 69). Determinism preserved — sorted col + entity ordering, deterministic PAVA, integer rounding.
...
6. **Crossing minimization at track assignment.** Beyond non-overlap packing, choose tracks to minimize crossings with other edges. NP-hard exact; barycenter-style heuristic. Real win for dense channels but significant slice. Concrete case: `pure_star` left channel — four forward V's, FFD-by-yMin packs `{customer, promotion, employee}` on track 0 and `{channel}` on track 1; channel↔employee overlap then forces two H×V crossings regardless of which side channel sits on. Within 2 tracks the crossings are unavoidable (interval graph forces it); fix is either smarter assignment with possible track widening, or a heuristic that adds a 3rd track when 2 tracks would force crossings.
...
8. **Version stamp for `--render-markdown` output (post-npm-publish only).** Insert `<!-- dbsketch-version: X.Y.Z -->` as a separate HTML comment immediately before each `dbsketch-rendered` block. Invisible on GitHub; lets CI or the pre-commit hook skip re-rendering when the installed version matches the stamp, and surface a warning when it doesn't. Do not add before npm publishing — the version number is meaningless pre-release, and the stamp causes every version bump to dirty every rendered markdown file even when diagram output is unchanged.
9. **PyPI wrapper for Python/data-engineer audience (post-npm-publish).** Data engineers with SQL schemas are Python-first and will bounce at `npm install`. Publish `dbsketch` on PyPI as a thin wrapper that bundles the pre-built Node binary + compiled CLI JS in platform-specific wheels (`linux-x86_64`, `linux-aarch64`, `macos-arm64`, `macos-x86_64`, `win-amd64`). A Python shim (`__main__.py`) resolves the binary and `subprocess.run`s it — `pip install dbsketch` then `dbsketch schema.sql` just works with no Node dependency visible to the user. Precedent: `pyright`, `tailwindcss`, `prettier` all use this pattern. Main cost is the CI wheel-build matrix and keeping the PyPI and npm versions in lockstep. Do after npm publishing so there's a stable versioned CLI artifact to wrap.
